    ...
    with st.sidebar:
        ...
        if uploaded_file is not None and st.session_state.get("last_loaded_file") != uploaded_file.name:
            try:
                ...
                if available_roles:
                    # Auto-switch role if not matching
        ...
        if st.button("֎Review Response with AI"):
            if not user_input.strip():
                ...
                with st.spinner("Analyzing with GPT..."):
                    try:
                        st.session_state.suggestion = get_gpt_feedback(user_input, current_key, section, selected_role)
                        st.success("AI Suggestions generated!")
                    except openai.APITimeoutError:
                        st.warning("⚠️ OpenAI request timed out. Try again or simplify your input.")
                        st.session_state.suggestion = ""
        ...
        if st.button("🔍 Review Entire Report with AI"):
            if not has_responses(st.session_state.responses[selected_role]):
                ...
                with st.spinner("Analyzing full report with GPT..."):
                    try:
                        st.session_state.full_feedback = get_full_report_feedback(
                            st.session_state.responses[selected_role], competency_sections, selected_role)
                        st.success("Full report review completed!")
                    except openai.APITimeoutError:
                        st.warning("⚠️ OpenAI request timed out. Try again or simplify your input.")
                        st.session_state.suggestion = ""
